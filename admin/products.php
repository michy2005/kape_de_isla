...
if (isset($_GET['delete_primary'])) {
    ...
    if ($check->fetchColumn() == 0) {
        ...
        header("Location: products.php?error=primary_not_empty");
        exit;
    }
}
...
if (isset($_GET['delete_sub'])) {
    ...
    if ($check->fetchColumn() == 0) {
        ...
        header("Location: products.php?error=sub_not_empty");
        exit;
    }
}
...
<head>
    ...
    <style>
        ...
        .prime-gradient {
            background: linear-gradient(135deg, #CA8A4B 0%, #a66d35 100%);
        }

        .sub-header-line {
            height: 1px;
            background: linear-gradient(90deg, rgba(202, 138, 75, 0.3) 0%, rgba(202, 138, 75, 0) 100%);
        }
        ...
        .swal2-popup {
            ...
            backdrop-filter: blur(16px);
            border: 1px solid rgba(202, 138, 75, 0.2) !important;
        ...
        .swal2-confirm {
            background-color: #CA8A4B !important;
...
<body class="text-stone-300 min-h-screen p-6 md:p-12">
    ...
    <script>
        ...
        window.onload = () => {
            ...
            if (urlParams.has('success')) {
            ...
            } else if (urlParams.has('updated')) { // ADDED THIS
                trigger = true;
                title = 'Product Updated';
                text = 'The roast profile has been successfully modified.';
                ...
                icon = 'error';
                title = 'Oops...';
                // Check specific error types or show general error
                if (urlParams.get('error') === 'primary_not_empty' || urlParams.get('error') === 'sub_not_empty') {
                    text = 'Cannot delete a section that still contains active items.';
                } else {
                    text = 'Something went wrong with the update. Please try again.';
                }
            }
    ...
    <div class="max-w-7xl mx-auto">
        <header class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-16">
            <div>
                <a href="dashboard.php"
                    class="text-[#CA8A4B] text-[9px] font-bold uppercase tracking-[0.4em] flex items-center gap-2 mb-3 opacity-80 hover:opacity-100 transition">
            ...
            <div class="flex flex-wrap items-center gap-4 w-full xl:w-auto">
                <div class="relative flex-1 md:flex-none md:w-64">
                    <input type="text" id="productSearch" onkeyup="searchProducts()" placeholder="SEARCH ROASTS..."
                        class="w-full bg-black/40 border border-white/10 text-white text-[10px] font-bold uppercase tracking-widest px-12 py-4 rounded-2xl outline-none focus:border-[#CA8A4B] transition-all">
                    <i data-lucide="search" class="w-4 h-4 absolute left-5 top-1/2 -translate-y-1/2 text-stone-500"></i>
                </div>

                <div class="relative flex-1 md:flex-none">
                    <select onchange="window.location.href='?filter_cat='+this.value"
                        class="w-full bg-black/40 border border-white/10 text-white text-[10px] font-bold uppercase tracking-widest px-6 py-4 rounded-2xl outline-none focus:border-[#CA8A4B] appearance-none cursor-pointer pr-12">
                    ...
                    <i data-lucide="filter"
                        class="w-3 h-3 absolute right-5 top-1/2 -translate-y-1/2 text-stone-500 pointer-events-none"></i>
                </div>

                <button onclick="openPrimaryModal()"
                    class="group flex items-center gap-3 border border-white/10 text-white px-7 py-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:border-[#CA8A4B]/50 transition-all bg-white/5">
        ...
        <div class="space-y-20">
            ...
            if ($filter_id !== 'all')
                $query .= " WHERE id = " . intval($filter_id);
                ...
                <section class="primary-section">
                    <div class="flex justify-between items-center mb-8 border-b border-white/5 pb-4 group">
                        <div class="flex items-center gap-6">
                            <h2 class="text-3xl font-serif text-white italic tracking-wide">
                                <?= htmlspecialchars($p_cat['name']) ?>
                            </h2>
                            <div class="flex gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onclick="openPrimaryModal(<?= $p_cat['id'] ?>, '<?= addslashes($p_cat['name']) ?>')"
                                    class="text-stone-500 hover:text-white transition-colors"><i data-lucide="edit-2"
                                        class="w-4 h-4"></i></button>
                                <button onclick="confirmDelete('?delete_primary=<?= $p_cat['id'] ?>', 'Primary Category')"
                                    class="text-stone-500 hover:text-red-500 transition-colors"><i data-lucide="trash-2"
                                        class="w-4 h-4"></i></button>
                            </div>
                        </div>
                        <button onclick="openSubModal(<?= $p_cat['id'] ?>, '<?= addslashes($p_cat['name']) ?>')"
                            class="flex items-center gap-2 text-stone-500 hover:text-[#CA8A4B] text-[10px] font-bold uppercase tracking-[0.2em] transition-colors">
                            ...
                            <div class="relative group/sub sub-section">
                                <div class="flex justify-between items-center mb-8">
                                    <div class="flex-1 flex items-center gap-4">
                                        <h3 class="text-[11px] font-bold text-[#CA8A4B] uppercase tracking-[0.3em] mb-1">
                                            <?= htmlspecialchars($s_cat['name']) ?>
                                        </h3>
                                        <div class="flex gap-3 opacity-0 group-hover/sub:opacity-100 transition-opacity">
                                            <button
                                                onclick="openSubModal(<?= $p_cat['id'] ?>, '<?= addslashes($p_cat['name']) ?>', <?= $s_cat['id'] ?>, '<?= addslashes($s_cat['name']) ?>')"
                                                class="text-stone-600 hover:text-white transition-colors"><i
                                                    data-lucide="edit-2" class="w-3 h-3"></i></button>
                                            <button onclick="confirmDelete('?delete_sub=<?= $s_cat['id'] ?>', 'Sub-category')"
                                                class="text-stone-600 hover:text-red-500 transition-colors"><i
                                                    data-lucide="trash-2" class="w-3 h-3"></i></button>
                                        </div>
                                        <div class="sub-header-line flex-1"></div>
                                    </div>
                                    <a href="add_product.php?sub_id=<?= $s_cat['id'] ?>"
                                        class="ml-6 flex items-center gap-2 bg-white/5 hover:bg-[#CA8A4B] text-white px-5 py-2.5 rounded-xl text-[9px] font-bold uppercase tracking-widest transition-all border border-white/5">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 product-container">
                                    ...
                                    foreach ($all_products as $p):
                                        ...
                                        ?>
                                        <div
                                            class="product-card glass-dark p-5 rounded-[2.5rem] border-white/5 flex gap-5 items-center group/prod hover:bg-white/[0.07] transition-all duration-500 hover:-translate-y-1">
                                            <div class="relative">
                                                <img src="../<?= $p['image_url'] ?>"
                                                    class="w-16 h-16 rounded-2xl object-cover bg-black/40 shadow-2xl <?= $is_out_of_stock ? 'grayscale opacity-50' : '' ?>">
                                                <?php if ($is_out_of_stock): ?>
                                                    <div class="absolute inset-0 flex items-center justify-center">
                                                        <span
                                                            class="bg-red-600 text-[7px] text-white font-bold uppercase px-2 py-0.5 rounded-full">Empty</span>
                                                    </div>
                                                <?php endif; ?>
                                            </div>
                                            <div class="flex-1 min-w-0">
                                                <h4 class="product-name text-white text-sm font-bold truncate mb-1">
                                                    <?= htmlspecialchars($p['name']) ?>
                                                </h4>
                                                <div class="flex items-center gap-3">
                                                    <span
                                                        class="text-[#CA8A4B] text-xs font-bold">₱<?= number_format($p['price'], 2) ?></span>
                                                    <span id="stock-<?= $p['id'] ?>"
                                                        class="stock-display text-[9px] <?= $is_out_of_stock ? 'text-red-500 font-bold' : 'text-stone-500' ?> uppercase">
                                                        Stock: <?= $p['stock'] ?>
                                                    </span>
                                                </div>
                                            </div>
                                            <div
                                                class="flex flex-col gap-3 opacity-0 group-hover/prod:opacity-100 transition-all duration-300">
                                                <a href="edit_product.php?id=<?= $p['id'] ?>"
                                                    class="text-stone-400 hover:text-white transition-colors"><i
                                                        data-lucide="edit-3" class="w-4 h-4"></i></a>
                                                <button onclick="confirmDelete('?delete=<?= $p['id'] ?>', 'Product')"
                                                    class="text-stone-600 hover:text-red-500 transition-colors"><i
    ...
    <div id="primaryModal"
        class="fixed inset-0 bg-black/90 backdrop-blur-md hidden z-[110] flex items-center justify-center p-4">
        <div class="glass-dark p-10 rounded-[3rem] max-w-sm w-full border-white/10 shadow-2xl">
            ...
            <form method="POST">
                ...
                <input type="text" name="cat_name" id="primaryEditName" placeholder="CATEGORY NAME" required
                    class="w-full bg-black/40 border border-white/10 rounded-2xl p-5 text-sm text-white mb-6 outline-none focus:border-[#CA8A4B] transition-colors uppercase">
                <div class="flex gap-4">
                    <button type="button" onclick="closeModal('primaryModal')"
                        class="flex-1 py-4 text-stone-500 text-[10px] font-bold uppercase tracking-widest">Cancel</button>
                    <button type="submit" name="save_primary"
                        class="flex-1 py-4 prime-gradient text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest">Save</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="subModal"
        ...
        <div class="glass-dark p-10 rounded-[3rem] max-w-sm w-full border-white/10 shadow-2xl">
            ...
            <form method="POST">
                ...
                <input type="text" name="sub_name" id="subEditName" placeholder="SUB-CATEGORY NAME" required
                    class="w-full bg-black/40 border border-white/10 rounded-2xl p-5 text-sm text-white mb-6 outline-none focus:border-[#CA8A4B] transition-colors uppercase">
                <div class="flex gap-4">
                    <button type="button" onclick="closeModal('subModal')"
                        class="flex-1 py-4 text-stone-500 text-[10px] font-bold uppercase tracking-widest">Cancel</button>
                    <button type="submit" name="save_sub"
                        class="flex-1 py-4 prime-gradient text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest">Save</button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        function syncStockLive() {
            fetch('api/get_all_stock.php')
                .then(response => response.json())
                .then(data => {
                    data.forEach(item => {
                        const stockLabel = document.getElementById(`stock-${item.id}`);
                        if (stockLabel) {
                            // Update the text
                            stockLabel.innerText = `Stock: ${item.stock}`;

                            // Update the styling live
                            if (item.stock <= 0) {
                                stockLabel.classList.remove('text-stone-500');
                                stockLabel.classList.add('text-red-500', 'font-bold');
                                // Find the parent card and image to apply grayscale if empty
                                const card = stockLabel.closest('.product-card');
                                const img = card.querySelector('img');
                                img.classList.add('grayscale', 'opacity-50');
                            } else {
                                stockLabel.classList.add('text-stone-500');
                                stockLabel.classList.remove('text-red-500', 'font-bold');
                                const card = stockLabel.closest('.product-card');
                                const img = card.querySelector('img');
        ...
        setInterval(syncStockLive, 5000);
    </script>
</body>
