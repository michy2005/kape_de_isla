...
<head>
    ...
    <style>
        ...
        .status-badge { display: flex; align-items: center; gap: 6px; padding: 6px 14px; border-radius: 12px; font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.05em; }
        
        .status-pending { background: rgba(202, 138, 75, 0.1); color: #CA8A4B; border: 1px solid rgba(202, 138, 75, 0.2); }
        .status-brewing { background: rgba(234, 179, 8, 0.1); color: #fbbf24; border: 1px solid rgba(234, 179, 8, 0.2); }
        .status-outfordelivery { background: rgba(56, 189, 248, 0.1); color: #38bdf8; border: 1px solid rgba(56, 189, 248, 0.2); }
        ...
        .item-scroll::-webkit-scrollbar-thumb { background: rgba(202, 138, 75, 0.3); border-radius: 10px; }
    </style>
</head>
    ...
    <main class="max-w-6xl mx-auto px-6 pt-32 pb-20">
        <div class="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div>
                ...
                <p class="text-[#CA8A4B] text-[10px] tracking-[0.4em] uppercase font-bold mt-2">Track your caffeine journey</p>
            </div>
            <button onclick="handleGoBack()" class="text-stone-500 hover:text-white text-[10px] tracking-widest font-bold uppercase flex items-center gap-2 transition cursor-pointer">
        ...
        <?php if (count($orders) > 0): ?>
            <div class="space-y-8">
                <?php foreach ($orders as $order): ?>
                    <div class="glass-dark rounded-[2.5rem] overflow-hidden border border-white/5 hover:border-[#CA8A4B]/30 transition-all duration-500 group">
                        <div class="p-8">
                            <div class="flex flex-col lg:flex-row gap-8">
                                ...
                                <div class="lg:w-1/4 space-y-4">
                                    <div class="flex flex-wrap gap-3">
                                        <?php 
                                            ...
                                            $icon = 'clock';
                                            if($stat == 'delivered') $icon = 'check-circle-2';
                                            if($stat == 'brewing') $icon = 'flame';
                                            if($stat == 'outfordelivery') $icon = 'truck';
                                        ?>
                                    ...
                                    <div>
                                        <h3 class="text-white font-serif text-xl italic">TXN-<?= date('Ymd', strtotime($order['created_at'])) ?>-<?= $order['id'] ?></h3>
                                        <p class="text-stone-500 text-[10px] uppercase tracking-tighter mt-1"><?= date('F j, Y • g:i A', strtotime($order['created_at'])) ?></p>
                                    </div>
                                    <div class="pt-2">
                                        <p class="text-stone-400 text-[11px] leading-relaxed flex gap-2">
                                            ...
                                            <span>
                                                ...
                                                <span class="text-[#CA8A4B] uppercase text-[9px] font-bold"><?= $order['label'] ?></span>
                                            </span>
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="lg:w-2/4 bg-black/20 rounded-3xl p-6 border border-white/5">
                                    <p class="text-[9px] text-stone-500 uppercase tracking-widest mb-4 font-bold">Items in this Brew</p>
                                    <div class="item-scroll space-y-4 overflow-y-auto max-h-[160px] pr-2">
                                        ...
                                        <div class="flex items-center gap-4 group/item">
                                            <div class="w-12 h-12 rounded-xl overflow-hidden bg-stone-900 border border-white/10 shrink-0">
                                                <img src="<?= $display_img ?>" class="w-full h-full object-cover opacity-80 group-hover/item:opacity-100 transition">
                                            </div>
                                            <div class="flex-1">
                                                ...
                                                <span class="text-[9px] text-[#CA8A4B] uppercase tracking-widest"><?= $item['mode'] ?> Mode</span>
                                            </div>
                                            <p class="text-sm font-serif text-stone-400">₱<?= number_format($item['price_at_purchase'] * $item['quantity'], 2) ?></p>
                                        </div>
                                        <?php endwhile; ?>
                                    </div>
                                </div>
                                ...
                                <div class="lg:w-1/4 text-right flex flex-col justify-between border-l border-white/5 pl-8">
                                    ...
                                    <?php if($order['status'] == 'Delivered'): ?>
                                        <button class="w-full py-3 bg-[#4ade80]/10 hover:bg-[#4ade80] text-[#4ade80] hover:text-[#1a0f0a] border border-[#4ade80]/20 rounded-xl text-[9px] uppercase tracking-widest font-bold transition flex items-center justify-center gap-2">
                                            View Receipt <i data-lucide="receipt" class="w-3 h-3"></i>
                                        </button>
                                    <?php else: ?>
                                        <button class="w-full py-3 bg-white/5 hover:bg-[#CA8A4B] hover:text-white border border-white/10 rounded-xl text-[9px] uppercase tracking-widest font-bold transition flex items-center justify-center gap-2">
        ...
        <?php else: ?>
            <div class="glass-dark p-20 rounded-[4rem] text-center border border-white/5">
                ...
                <a href="index.php" class="inline-block bg-[#CA8A4B] text-white px-10 py-4 rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-[#b07840] transition">
                    Explore Menu
                </a>
            </div>
        <?php endif; ?>
