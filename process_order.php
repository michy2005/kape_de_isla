...
if ($_SERVER["REQUEST_METHOD"] == "POST" && count($cart_items) > 0) {
    ...
    if (!$address_id) {
        die("Order failed: No delivery address selected.");
    }

    try {
        $pdo->beginTransaction();

        // 4. INSERT INTO main ORDERS table (Items column removed as per SQL update)
        $sql = "INSERT INTO orders (user_id, address_id, customer_name, total_amount, status, created_at) 
                VALUES (:uid, :aid, :name, :total, 'Pending', NOW())";
        ...
        // 5. INSERT INTO order_items AND DEDUCT STOCK
        foreach ($cart_items as $item) {
            // Save individual item details
            $itemSql = "INSERT INTO order_items (order_id, product_id, quantity, mode, price_at_purchase) 
                        VALUES (?, ?, ?, ?, ?)";
            ...
            $itemStmt->execute([
                ...
                $item['product_id'],
                $item['quantity'],
                $item['mode'], // This is the 'mode' (Iced/Hot)
                $item['price']
            ]);
...
<head>
    ...
    <meta http-equiv="refresh" content="10;url=my_orders.php">
    <style>
...
<body class="flex items-center justify-center min-h-screen p-6">
    <div class="glass p-12 md:p-16 rounded-[4rem] text-center max-w-lg border border-white/5 shadow-[0_30px_100px_rgba(0,0,0,0.8)]">
        ...
        <p class="text-stone-400 text-sm leading-relaxed mb-10 px-6">Your selection is being prepared. We'll bring the island taste to your doorstep shortly.</p>
        <div class="flex flex-col gap-4">
            <a href="my_orders.php" class="bg-[#CA8A4B] text-white py-5 rounded-2xl font-bold tracking-[0.2em] text-[10px] uppercase hover:bg-[#b07840] transition-all">Track My Order</a>
